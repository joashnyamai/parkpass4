 ...
 * - Creating bookings with atomic transactions
...
import { updateParkingAvailability } from './parkingService';

/**
 * Create a new booking with atomic transaction
 * Ensures parking space availability before booking
 */
export const createBooking = async (bookingData) => {
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      // If not found, try legacy parking_slots collection
      if (!spaceDoc.exists()) {
 ...
 */
export const watchAllBookings = (callback) => {
  const q = query(
    collection(db, 'ParkingHistory'),
    orderBy('createdAt', 'desc')
  );
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    console.error('Error watching all bookings:', error);
    callback([]);
  });
};
...
/**
 * Complete booking and release parking space
 */
export const completeBooking = async (bookingId, parkingSpaceId) => {
  try {
    await updateDoc(doc(db, 'ParkingHistory', bookingId), {
      status: 'completed',
      updatedAt: serverTimestamp()
    ...
    await updateParkingAvailability(parkingSpaceId, 1);
...
/**
 * Cancel booking and release parking space
 */
